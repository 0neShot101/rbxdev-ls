...
import { createInitializeResult, createServerConnection } from '@core/connection';
...
export const startServer = (state: ServerState): void => {
  ...
  const setupSourcemapWatcher = (sourcemapPath: string): void => {
    try {
      ...
      sourcemapWatcher.on('error', err => connection.console.log(`Sourcemap watcher error: ${err.message}`));
    } catch (err) {
