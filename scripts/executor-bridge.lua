...
serializeInstance = function(instance, depth)
	...
	for _, child in ipairs(instance:GetChildren()) do
		...
		if childNode ~= nil then table.insert(children, childNode) end
	end
...
local getGameTree = function(services)
	local tree = {}
...
local getTreeHash = function(tree)
	local parts = {}
...
local sendGameTreeIfChanged
sendGameTreeIfChanged = function()
	if connected == false then return end

	local tree = getGameTree()
	local hash = getTreeHash(tree)

	if hash == lastTreeHash then return end
...
MESSAGE_HANDLERS.requestGameTree = function(message)
	send({ type = 'gameTree', data = getGameTree(message.services) })
end
...
local setupAutoRefresh = function()
	...
	task.spawn(function()
		while true do
			...
			if connected == true then sendGameTreeIfChanged() end
		end
	end)
