...
export const collectHighlights = (chunk: Chunk): Map<string, HighlightLocation[]> => {
  ...
  walk(chunk, {
    ...
    'visitLocalFunction': node => {
      addHighlight(node.name, DocumentHighlightKind.Write);
    },
    ...
    'visitForNumeric': node => {
      addHighlight(node.variable, DocumentHighlightKind.Write);
    },
    'visitForGeneric': node => {
      for (const v of node.variables) addHighlight(v, DocumentHighlightKind.Write);
    },
    'visitIdentifier': node => {
      addHighlight(node, DocumentHighlightKind.Text);
    },
    'visitMemberExpression': node => {
      addHighlight(node.property, DocumentHighlightKind.Text);
    },
    'visitMethodCallExpression': node => {
      addHighlight(node.method, DocumentHighlightKind.Text);
    },
  });
